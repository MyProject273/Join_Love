...
func (processor *RedisTaskProcessor) ProcessTaskSendVerifyEmail(ctx context.Context, task *asynq.Task) error {
	...
	verifyUrl := fmt.Sprintf("http://localhost:8080/api/v1/verify_email?email_id=%s&secret_code=%s",
		verifyEmail.ID, verifyEmail.SecretCode)
	var subject, content string
	switch payload.Lang {
	case "vi":
		subject = "Chào mừng bạn đến với Join Love"
		content = fmt.Sprintf(`Xin chào,<br/>
		Cảm ơn bạn đã đăng ký!<br/>
		Vui lòng <a href="%s">bấm vào đây</a> để xác minh địa chỉ email của bạn.<br/>`, verifyUrl)
	case "en":
		fallthrough
	default:
		subject = "Welcome to Join Love"
